...
private:
    ...
    Eigen::IOFormat MyFmt(const int& Tab_N);


private:
    const bool m_DebugOn; // If true, a lot of std::cout
...
public:
    ForgetfulGlobalTrajectory(
        ...
        const Type& MaxSpeed,
        const Type& MaxThrust,
        const Type& MaxRollPitchRate,
        const Type& SamplingTime,
        const int& Gurobi_NumericFocus,
        const Type& NonDimRange_Temporal,
        const Type& NonDimRange_Spatial,
        const bool& DebugOn);
...
ForgetfulGlobalTrajectory<Type>::ForgetfulGlobalTrajectory(
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& Waypoints,
    const int& PolynomialOrder,
    const int& ContinuousDifferentiabilityOrder,
    const Type& MaxSpeed,
    const Type& MaxThrust,
    const Type& MaxRollPitchRate,
    const Type& SamplingTime,
    ...
    m_Wp_W_N {Waypoints.size()},
    m_Wp_T_N {m_Wp_W_N + 1},
    m_Spl_N {m_Wp_W_N},
    m_Spl_PolyCff_N {PolynomialOrder + 1},
    m_TrajPolyCff_N {m_Spl_N * m_Spl_PolyCff_N},
    
    m_ContDiff_N {ContinuousDifferentiabilityOrder},
    m_Traj_MaxSpeed_Required {MaxSpeed},
    m_Traj_MaxThrust_Required {MaxThrust},
    m_Traj_MaxRollPitchRate_Required {MaxRollPitchRate},
    m_Traj_SamplingTime_Dim {SamplingTime},

    
    m_GurobiNumericFocus{Gurobi_NumericFocus},
    m_NonDimMinMax_T{NonDimRange_Temporal},
    m_NonDimMinMax_W{NonDimRange_Spatial},
    ...
    m_Successful{false}
{
                                                                                        ...
                                                                                        //    << std::endl 
                                                                                        //    << std::endl
                                                                                        //    << " FORGETFUL GLOBAL TRAJECTORY" << std::endl
                                                                                        //    << std::endl;
    
    // Set spatial coordinates from input
    for (int Wp_i = 0; Wp_i < m_Wp_W_N; Wp_i++)
    {
        m_Wp_X_Dim(Wp_i) = Waypoints[Wp_i].cast<Type>().x();
        m_Wp_Y_Dim(Wp_i) = Waypoints[Wp_i].cast<Type>().y();
        m_Wp_Z_Dim(Wp_i) = Waypoints[Wp_i].cast<Type>().z();
    }
                                                                                        // std::cout << "\t// m_Wp_X_Dim:" << m_Wp_X_Dim.transpose().format(MyFmt(2)) << std::endl << std::endl;
                                                                                        // std::cout << "\t// m_Wp_Y_Dim:" << m_Wp_Y_Dim.transpose().format(MyFmt(2)) << std::endl << std::endl;
                                                                                        // std::cout << "\t// m_Wp_Z_Dim:" << m_Wp_Z_Dim.transpose().format(MyFmt(2)) << std::endl << std::endl; 
                                                                                    
    // Compute preliminary temporal coordinates
    m_Wp_T_Dim(0) = 0;
    Type Spl_DurationSum = 0;
    for (int Spl_i = 0; Spl_i < m_Spl_N; Spl_i++)
    {
        const Eigen::Vector3d& Wp_0 = Waypoints[Spl_i];                                 // std::cout << "\t\t// Wp_0:" << Wp_0.transpose().format(MyFmt(2)) << std::endl << std::endl;
        const Eigen::Vector3d& Wp_1 = Waypoints[(Spl_i + 1) % m_Spl_N];                 // std::cout << "\t\t// Wp_1:" << Wp_1.transpose().format(MyFmt(2)) << std::endl << std::endl;

        const Type Spl_Distance = (Wp_1.cast<Type>() - Wp_0.cast<Type>()).norm();       // std::cout << "\t\t// Spl_Distance: " << Spl_Distance << std::endl << std::endl;
        const Type Spl_Duration = Spl_Distance / MaxSpeed;                              // std::cout << "\t\t// Spl_Duration: " << Spl_Duration << std::endl << std::endl;
        
        Spl_DurationSum += Spl_Duration;                                                
        m_Wp_T_Dim(Spl_i + 1) = Spl_DurationSum;                                        
                                                                                        // std::cout << "\t\t// m_Wp_T_Dim(Spl_i + 1): " << m_Wp_T_Dim(Spl_i + 1) << std::endl << std::endl;
    }
    ...
    // Dedimensionalize temporal and spatial coordinates
        dedimensionalizeCoordinates(m_Wp_T_Dim, m_NonDimMinMax_T, 
            m_Wp_T_NonDim, m_Shift_T, m_Scale_T);
        ...
        dedimensionalizeCoordinates(m_Wp_X_Dim, m_NonDimMinMax_W, 
            m_Wp_X_NonDim, m_Shift_X, m_Scale_X);                                       // std::cout << "\t// m_Shift_X: " << m_Shift_X << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Scale_X: " << m_Scale_X << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Wp_X_NonDim:" << m_Wp_X_NonDim.transpose().format(MyFmt(2)) << std::endl << std::endl;
        dedimensionalizeCoordinates(m_Wp_Y_Dim, m_NonDimMinMax_W, 
            m_Wp_Y_NonDim, m_Shift_Y, m_Scale_Y);                                       // std::cout << "\t// m_Shift_Y: " << m_Shift_Y << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Scale_Y: " << m_Scale_Y << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Wp_Y_NonDim:" << m_Wp_Y_NonDim.transpose().format(MyFmt(2)) << std::endl << std::endl;
        dedimensionalizeCoordinates(m_Wp_Z_Dim, m_NonDimMinMax_W, 
            m_Wp_Z_NonDim, m_Shift_Z, m_Scale_Z);                                       // std::cout << "\t// m_Shift_Z: " << m_Shift_Z << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Scale_Z: " << m_Scale_Z << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Wp_Z_NonDim:" << m_Wp_Z_NonDim.transpose().format(MyFmt(2)) << std::endl << std::endl;
                                                                                        // plotWaypoints(m_Wp_X_NonDim, m_Wp_Y_NonDim);
    
        m_Traj_Duration_NonDim = m_Wp_T_NonDim(m_Wp_T_N - 1) - m_Wp_T_NonDim(0);        // std::cout << "\t// m_Traj_Duration_NonDim: " << m_Traj_Duration_NonDim << std::endl << std::endl; 
        m_Traj_SamplingTime_NonDim = m_Traj_SamplingTime_Dim / m_Scale_T;               // std::cout << "\t// m_Traj_SamplingTime_Dim: " << m_Traj_SamplingTime_Dim << std::endl << std::endl; 
                                                                                        // std::cout << "\t// m_Traj_SamplingTime_NonDim: " << m_Traj_SamplingTime_NonDim << std::endl << std::endl;
        m_Traj_T_N = 1 
            + static_cast<int>(m_Traj_Duration_NonDim / m_Traj_SamplingTime_NonDim);    
                                                                                        // std::cout << "\t// m_Traj_T_N: " << m_Traj_T_N << std::endl << std::endl;




    
    // m_TrajPolyCff_N >< m_TrajPolyCff_N
    const MAT QP_ObjMat = buildObjMat();                                                // std::cout << "\t// QP_ObjMat:\n" << QP_ObjMat.format(MyFmt(2)) << std::endl << std::endl;
    // m_Spl_N * (m_ContDiff_N + 2) >< 1
    const VEC QP_EquVec_X = buildEquVec(m_Wp_X_NonDim);                                 // std::cout << "\t// QP_EquVec_X:" << QP_EquVec_X.transpose().format(MyFmt(2)) << std::endl << std::endl;
    // m_Spl_N * (m_ContDiff_N + 2) >< 1
    const VEC QP_EquVec_Y = buildEquVec(m_Wp_Y_NonDim);                                 // std::cout << "\t// QP_EquVec_Y:" << QP_EquVec_Y.transpose().format(MyFmt(2)) << std::endl << std::endl;
    // m_Spl_N * (m_ContDiff_N + 2) >< 1 
    const VEC QP_EquVec_Z = buildEquVec(m_Wp_Z_NonDim);                                 // std::cout << "\t// QP_EquVec_Z:" << QP_EquVec_Z.transpose().format(MyFmt(2)) << std::endl << std::endl;


    // Set names of decision variables
    std::vector<std::string> QP_DecVar_Names_X;
    std::vector<std::string> QP_DecVar_Names_Y;
    std::vector<std::string> QP_DecVar_Names_Z;
    for (int Spl_i = 0; Spl_i < m_Spl_N; Spl_i++)
        for (int SplCff_i = 0; SplCff_i < m_Spl_PolyCff_N; SplCff_i++)
        {
            QP_DecVar_Names_X.push_back("X-Spline_" + std::to_string(Spl_i) + ": c" + std::to_string(SplCff_i));
            QP_DecVar_Names_Y.push_back("Y-Spline_" + std::to_string(Spl_i) + ": c" + std::to_string(SplCff_i));
            QP_DecVar_Names_Z.push_back("Z-Spline_" + std::to_string(Spl_i) + ": c" + std::to_string(SplCff_i));
        }
    ...
    {                                                                                   //std::cout << "\t\t// Iter_i: " << Iter_i << std::endl << std::endl;                                                                                   
        ...
        {                                                                               //std::cout << "\t\t\t// Spl_i: " << Spl_i << std::endl << std::endl; 
            ...
            m_Successful = computeTrajectory(
                ...
                Gradient_Wp_T_NonDim); if (!m_Successful) break;
        ...
        for (int _=0; _<20; _++)
        {
            ...
            m_Successful = computeTrajectory(
                ...
                QP_ObjMat, QP_EquVec_X, QP_EquVec_Y, QP_EquVec_Z,
                QP_DecVar_Names_X, QP_DecVar_Names_Y, QP_DecVar_Names_Z,
                BLS_Wp_T_NonDim); if (!m_Successful) break;
        ...
        m_Successful = computeTrajectory(
            ...
            QP_DecVar_Names_Y,
            QP_DecVar_Names_Z,
            m_Wp_T_NonDim); if (!m_Successful) break;
...
template <typename Type>
Eigen::IOFormat 
ForgetfulGlobalTrajectory<Type>::MyFmt(
    const int& Tab_N
){
    std::string RowPrefix = "";
    for (int Tab_i = 0; Tab_i < Tab_N; Tab_i++) { RowPrefix += "\t"; }
    RowPrefix += "[ ";
