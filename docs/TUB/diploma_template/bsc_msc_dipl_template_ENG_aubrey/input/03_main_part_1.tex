...
\subsection*{Transformation between the global and the local reference system}
The drone position
$
...
\begin{align}
    ...
    .
\end{align}
%\url{https://reader.elsevier.com/reader/sd/pii/B9780124158429000022?token=80D28F49695ADE2F8FBA3717576FA6BF850872EF274529E07574A99C62EE08BB68A4FB939F4877A1FAB29C3BAB785D6D&originRegion=eu-west-1&originCreation=20220617063425}
...
\subsection*{Transformation between the local and the image reference system}
The horizontal
$
...
\begin{align} \label{equ:local_to_image_transformation}
    T_\textbf{IL}
    :\ 
    &\mathbb{R}^3 \rightarrow \mathbb{R}_{[\text{-}1, 1]}^2
    \nonumber \\
    &{}_\textbf{L} \underline p 
    =
    \begin{bmatrix}
    ...
    \begin{bmatrix}
        \max \left\{
            ...
            \min \left\{
                - \frac
                    {1}
                    {\frac{1}{2}\phi^\text{user}_{z,\text{camera}}}
                \mathrm{atan2}\left(
                    {}_\textbf{L} y
                    ,
                    {}_\textbf{L} x
        ...
        \max \left\{
            ...
            \min \left\{
                \frac
                    {1}
                    {\frac{1}{2}\phi^\text{user}_{y,\text{camera}}}
                \mathrm{atan2}\left(
                    {}_\textbf{L} z
                    ,
                    \left\| 
    ...
    .
\end{align}
The above transformation can be interpreted as a 
projection of a point onto the image plane
obtained by
normalizing the yaw
$
\mathrm{atan2}\left(
...
\right)
$
angle of the vector from
$
{}_\textbf{L} \underline 0
$
to
$
{}_\textbf{L} \underline p 
$ 
with the half of the horizontal 
$
\phi^\text{user}_{z,\text{camera}}
$ 
...
Because the above transformation 
is not bijective %https://en.wikipedia.org/wiki/Bijection
and hence not invertible,
the transformation of a point from the image to the local reference system
requires a backprojection length $d$ as additional information
\begin{align} \label{eq:image_to_local_transformation}
    ...
    {}_\textbf{L}\phi_z
    = 
    - \frac{1}{2} \phi_{z,\text{camera}}^\text{user} \cdot {}_\textbf{I} x
    %\nonumber \\
    ,\ 
    {}_\textbf{L}\phi_y
    = 
    \frac{1}{2} \phi_{y,\text{camera}}^\text{user} \cdot {}_\textbf{I} y.
\end{align}
Note that $\odot$ denotes the Hadamard product
